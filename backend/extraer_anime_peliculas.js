...
const SECTION_ID = 3; // ID de la sección "Anime Películas"
const TOKEN = 's79ccz2DiRze6UsTTdG7';

// Construcción de rutas absolutas
const pathToFrontend = path.resolve(__dirname, '../frontend');
const OUTPUT_JSON = path.join(pathToFrontend, 'public/data/anime_peliculas.json');
const IMG_DIR = path.join(pathToFrontend, 'public/img/anime_peliculas');
...
async function main() {
  try {
    ...
    for (const item of Array.isArray(videos) ? videos : [videos]) {
      ...
      const thumbPath = `/img/anime_peliculas/${thumbFilename}`;
    ...
    console.log('✅ Anime Películas extraídas y guardadas exitosamente');
  } catch (err) {

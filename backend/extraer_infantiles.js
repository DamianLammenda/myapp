...
const SECTION_ID = 11; // Cambiado a la sección Películas Infantiles
const TOKEN = 's79ccz2DiRze6UsTTdG7';

// Construcción de rutas absolutas
const pathToFrontend = path.resolve(__dirname, '../frontend');
const OUTPUT_JSON = path.join(pathToFrontend, 'public/data/peliculas_infantiles.json');  // Archivo JSON distinto
const IMG_DIR = path.join(pathToFrontend, 'public/img/peliculas_infantiles');          // Carpeta distinta para imágenes
...
async function main() {
  try {
    ...
    if (!videos || (Array.isArray(videos) && videos.length === 0)) {
      console.error('❌ No se encontraron películas infantiles en la sección');
      return;
    }
    ...
    for (const item of Array.isArray(videos) ? videos : [videos]) {
      ...
      const thumbPath = `/img/peliculas_infantiles/${thumbFilename}`;
    ...
    console.log('✅ Películas infantiles extraídas y guardadas exitosamente');
  } catch (err) {

...
const SECTION_ID = 15;  // <-- ID de Series Retro
const TOKEN = 's79ccz2DiRze6UsTTdG7';
const OUTPUT_JSON = path.resolve(__dirname, '../frontend/public/data/retro.json');
const IMG_DIR = path.resolve(__dirname, '../frontend/public/img/retro');
...
async function main() {
  try {
    ...
    const result = await fetchRetroData();

    const directories = result?.MediaContainer?.Directory;

    if (!directories || directories.length === 0) {
      console.error('❌ No se encontraron series retro en la sección');
      return;
    }
    ...
    for (const item of Array.isArray(directories) ? directories : [directories]) {
      ...
      const thumbPath = `/img/retro/${thumbFilename}`;
    ...
    await fs.outputJson(OUTPUT_JSON, retroList, { spaces: 2 });
    console.log('✅ Datos de series retro extraídos y guardados exitosamente');
  } catch (err) {

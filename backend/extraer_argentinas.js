...
const SECTION_ID = 10; // Cambiado al ID de Películas Argentinas
const TOKEN = 's79ccz2DiRze6UsTTdG7';

// Construcción de rutas absolutas
const pathToFrontend = path.resolve(__dirname, '../frontend');
const OUTPUT_JSON = path.join(pathToFrontend, 'public/data/argentinas.json');
const IMG_DIR = path.join(pathToFrontend, 'public/img/argentinas');
...
async function main() {
  try {
    ...
    for (const item of Array.isArray(videos) ? videos : [videos]) {
      ...
      const thumbPath = `/img/argentinas/${thumbFilename}`;

...
const SECTION_ID = 2;  // <-- ID de Series
const TOKEN = 's79ccz2DiRze6UsTTdG7';
const OUTPUT_JSON = path.resolve(__dirname, '../frontend/public/data/series.json');
const IMG_DIR = path.resolve(__dirname, '../frontend/public/img/series');
...
async function main() {
  try {
    ...
    if (!directories || directories.length === 0) {
      console.error('❌ No se encontraron series en la sección');
      return;
    }
    ...
    for (const item of Array.isArray(directories) ? directories : [directories]) {
      ...
      const thumbPath = `/img/series/${thumbFilename}`;
    ...
    await fs.outputJson(OUTPUT_JSON, seriesList, { spaces: 2 });
    console.log('✅ Datos de series extraídos y guardados exitosamente');
  } catch (err) {
